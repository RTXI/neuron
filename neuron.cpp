...
static inline double alpha_m(double V) {
	...
	if (fabs(x / y) < 1e-6) return 0.1 * y * (1 - x / y / 2.0);
	else return 0.1 * x / (exp(x / y) - 1.0);
}
...
static inline double alpha_n(double V) {
	...
	if (fabs(x / y) < 1e-6) return 0.01 * y * (1 - x / y / 2.0);
	else return 0.01 * x / (exp(x / y) - 1.0);
}
...
void Neuron::execute(void) {
	...
	*/
	...
	for (int i = 0; i < steps; ++i) solve(period / steps, y);
